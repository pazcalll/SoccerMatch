...
public class LogActivity extends AppCompatActivity {
    ...
    private void loadHeader() {
        TextView matchResultTxt =
                (TextView)findViewById(R.id.matchResultTxt);
        TextView matchScoreTxt =
                (TextView)findViewById(R.id.matchScoreTxt);
        matchResultTxt.setText("Result: "+
                getIntent().getStringExtra("MATCH_RESULT"));
        matchScoreTxt.setText("Score: "+
                getIntent().getStringExtra("MATCH_SCORE"));
    }

    private void loadLogData() {
        ArrayList<LogItem> itemList = new ArrayList<>();
        LogAdapter itemArrayAdapter =
                new LogAdapter(R.layout.layout_log, itemList);
        itemArrayAdapter.setHasStableIds(true);
        logView.setAdapter(itemArrayAdapter);
        ArrayList<String> eventList =
                getIntent().getStringArrayListExtra("MATCH_EVENT");
